...
public class VPreguntaOp extends VPregunta
{	
	private List<VOpcio> vopcions = new ArrayList<VOpcio>();	
	...
	public VPreguntaOp(PreguntaOp pregunta, String id, HashMap respostes, HashMap hmEnq_codnum, HashMap hmPrg_codnum, HashMap hmTipus_prg, List<Integer> enq_codnum, String dni)
	{
...
//    	valueBinding = FacesContext.getCurrentInstance().getApplication().createValueBinding("#{GetModelCtl.respostesBean.respostes['"+this.getId()+"']}");
		...
		{			
			VOpcio v = new VOpcio(opcio.get(i),id+'o'+String.valueOf(i));
			vopcions.add(v);			
